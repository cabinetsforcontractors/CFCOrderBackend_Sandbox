...
def normalize_rta_supplier(from_name: str = "C&S",
    ...
    681 rta_products rows under bare 'C&S' with ZERO tokens). The 8/2 audit
    found the alias holds TWO owners' lines — SGCS/ESCS/CAWN/BSN/MSCS/WOCS
    are Cabinet & Stone but SAVNG routes to DL (7/14 ruling) — so the
    rename is PREFIX-SCOPED: pass prefixes=SGCS,ESCS,... A blanket rename
    (no prefixes) is still possible but shouldn't be used on C&S.
    dry_run=true (default) = the AUDIT. The real rename needs
    dry_run=false AND X-Allow-Destructive: yes (cleanup-door idiom)."""
    plist = [p.strip().upper() for p in (prefixes or "").split(",")
             if p.strip()]
    pf_sql = " AND split_part(product_sku, '-', 1) = ANY(%s)" if plist else ""

    def _args(base):
        return tuple(base) + ((plist,) if plist else ())
    ...
    with get_db() as conn:
        with conn.cursor() as cur:
            cur.execute(f"""SELECT COUNT(*),
                                  COUNT(*) FILTER (WHERE supplier_sku IS NOT NULL
                                                     AND supplier_sku != '')
                           FROM rta_products WHERE supplier = %s{pf_sql}""",
                        _args([from_name]))
            total, tokened = cur.fetchone()
            cur.execute(f"""SELECT split_part(product_sku, '-', 1) AS prefix,
                                  COUNT(*)
                           FROM rta_products WHERE supplier = %s{pf_sql}
                           GROUP BY 1 ORDER BY 2 DESC LIMIT 15""",
                        _args([from_name]))
            pref_rows = [{"prefix": r[0], "rows": r[1]} for r in cur.fetchall()]
            cur.execute(f"""SELECT product_sku FROM rta_products
                           WHERE supplier = %s{pf_sql} LIMIT 10""",
                        _args([from_name]))
            samples = [r[0] for r in cur.fetchall()]
        audit = {"from": from_name, "to": to_name, "prefix_filter": plist,
                 "rows": total, "with_token": tokened,
                 "prefixes": pref_rows, "samples": samples}
        if dry_run:
        ...
        if (x_allow_destructive or "").strip().lower() != "yes":
            return {"status": "error",
                    ...
                    "audit": audit}
        with conn.cursor() as cur:
            cur.execute(f"""UPDATE rta_products SET supplier = %s
