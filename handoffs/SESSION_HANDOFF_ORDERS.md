...
**Date:** 2026-04-03 (updated — SQUARE_ENVIRONMENT deferred, ADMIN_API_KEY clarified)
...
- SQUARE_ENVIRONMENT: NOT being set at Step 2 — Square promotion is deferred. square_sync.py hardcodes production Square URL regardless of env — checkout and sync talk to different Square environments. Needs a code fix in square_sync.py before Square is considered functional.
- ADMIN_API_KEY env var was stripped from Render — auth.py falls back to default "CFC2025" (NOT hardcoded, it is os.environ.get("ADMIN_API_KEY", "CFC2025")). Frontend sends CFC2026 → mismatch → 401 on all writes. Fix: add ADMIN_API_KEY=CFC2026 to Render env.
...
The frontend already sends `CFC2026` (api.js sha 68019f6e). The env var was stripped from Render so auth.py is falling back to `CFC2025`. Every write endpoint is rejecting requests until this is added.

1. Render dashboard → `cfc-backend-b83s.onrender.com` → Environment
2. Add `ADMIN_API_KEY=CFC2026`
3. Save → Manual deploy → watch logs
4. Then continue with Steps 3–6 below

⛔ Do NOT set SQUARE_ENVIRONMENT at this step — Square promotion is deferred.
...
Step 2 — Render: add ADMIN_API_KEY and repoint backend ← **DO THIS FIRST**
...
cfc-orders:auth.py — sha 795a0a76 — ⚠️ NOT hardcoded — os.environ.get("ADMIN_API_KEY", "CFC2025") — env var stripped → fallback CFC2025 → 401
...
- Set ADMIN_API_KEY=CFC2026 on Render at Step 2. Do NOT set SQUARE_ENVIRONMENT — deferred.
- auth.py is NOT hardcoded — uses os.environ.get("ADMIN_API_KEY", "CFC2025"). Env var was stripped → fallback to CFC2025 → 401. Fix = add the env var.
...
- square_sync.py hardcodes production Square URL — Square integration not fully functional until fixed. Deferred.
