...
`cfc-orders-frontend:README.md` (sha dd294bdd) — current component state and known issues.

**Current state (2026-04-03):**
- App.jsx v7.2.3 pushed to repo — sha ce6f739 ✅
- /health returns v6.2.0.
- ADMIN_API_KEY: NOT hardcoded — `os.environ.get("ADMIN_API_KEY", "CFC2025")`. If env var is absent on sandbox Render, falls back to CFC2025. Frontend sends CFC2026 → mismatch → 401 on all writes. Fix: add ADMIN_API_KEY=CFC2026 to CFCOrderBackend-Sandbox env vars on Render.
...
### Phase 7 Step 1 — DONE
- api.js updated: CFC2025 → CFC2026 — sha 68019f6e
...
3. Smoke test
4. DB migrations (idempotent): /add-rl-fields, /add-weight-column, /backfill-lifecycle
5. Full checklist
6. R+L end-to-end
...
cfc-orders-frontend:README.md — sha dd294bdd — ⚠️ READ FIRST
cfc-orders-frontend:src/App.jsx — sha ce6f739 — v7.2.3 ✅ pushed
...
cfc-orders:shippo_rates.py — sha 3e76f2a3 — 320 lines Shippo UPS/USPS rate quotes
...
- ADMIN_API_KEY not hardcoded — env var absent on sandbox Render → fallback CFC2025 → 401. Add CFC2026 before testing writes.
